...
export function buildTutorTurnPrompt(input: TutorModelInput): string {
  ...
  return [
    ...
    "Within the reply field only, write every mathematical expression as LaTeX: wrap inline math in single dollar signs (e.g. $\\frac{1}{2}$) and standalone equations or multi-line work in double dollar signs (e.g. $$x^2 + 1 = 0$$). Use LaTeX for all numbers-with-symbols math (fractions, exponents, roots, equations, variables), not for plain prose numbers. Do not use dollar signs for anything other than LaTeX math.",
