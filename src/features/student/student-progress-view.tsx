      ...
      <Surface elevated style={styles.surface}>
        <View style={styles.header}>
          <View style={styles.titleBlock}>
            <ProductText variant="eyebrow" color={Colors.light.primary}>Your learning</ProductText>
          ...
          <View style={styles.headerIcon} accessibilityLabel="Your progress" accessible>
            <ProductIcon name="target" size={22} color={Colors.light.primary} />
          </View>
        </View>
        ...
        {nextSkill ? (
          <View style={styles.nextStep}>
            <View style={styles.nextStepCopy}>
              <ProductText variant="eyebrow" color={Colors.light.primary}>A useful next step</ProductText>
        ...
        <View style={styles.sectionHeader}>
          ...
          <ProductText variant="caption" color={Colors.light.textSecondary}>Only you can see this</ProductText>
        </View>
            ...
            <View style={styles.memoryList}>
              {memory.map((item) => (
                <View key={item.id} style={styles.memoryItem}>
                  <View style={styles.memoryIcon}><ProductIcon name="memory" size={15} color={Colors.light.purpleText} /></View>
                  <View style={styles.memoryCopy}>
                    <ProductText variant="bodyMedium">A note from your practice</ProductText>
                    <ProductText variant="caption" color={Colors.light.textSecondary}>{item.text}</ProductText>
                    {item.createdAt ? <ProductText variant="caption" color={Colors.light.textSecondary}>{item.createdAt}</ProductText> : null}
                  </View>
                </View>
              ))}
            </View>
...
function Metric({ value, label }: { value: string; label: string }) {
  return (
    <View style={styles.metric}>
      ...
      <ProductText variant="caption" color={Colors.light.textSecondary}>{label}</ProductText>
    </View>
  );
}
...
function SkillRow({ skill, selected }: { skill: StudentProgressSkill; selected: boolean }) {
  ...
  return (
    <View style={[styles.skillRow, selected && styles.selectedSkill]} accessibilityLabel={`${skill.name}: ${studentSkillStatusLabel(status)}`}>
      <View style={styles.skillCopy}>
        ...
        <ProgressBar value={skill.mastery} color={status === 'strong' ? Colors.light.mintText : Colors.light.primary} />
        <ProductText variant="caption" color={Colors.light.textSecondary}>{studentSkillProgressLabel(skill)}</ProductText>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  headerIcon: { width: 46, height: 46, borderRadius: 16, backgroundColor: Colors.light.primarySoft, alignItems: 'center', justifyContent: 'center' },
  metrics: { flexDirection: 'row', gap: Spacing.two, flexWrap: 'wrap' },
  metric: { flex: 1, minWidth: 110, gap: 2, padding: Spacing.two, borderRadius: 14, backgroundColor: Colors.light.background },
  nextStep: { padding: Spacing.three, borderRadius: 17, backgroundColor: Colors.light.primarySoft, gap: Spacing.three },
  nextStepCopy: { gap: 5 },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: Spacing.two },
  skillList: { gap: Spacing.two },
  skillRow: { padding: Spacing.three, borderRadius: 16, backgroundColor: Colors.light.background, borderWidth: 1, borderColor: Colors.light.border },
  selectedSkill: { borderColor: Colors.light.primary },
  skillCopy: { gap: 8 },
  ...
  memoryItem: { flexDirection: 'row', alignItems: 'flex-start', gap: Spacing.two, padding: Spacing.two, borderRadius: 14, backgroundColor: Colors.light.purple },
  memoryIcon: { width: 28, height: 28, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: Colors.light.backgroundElement },
